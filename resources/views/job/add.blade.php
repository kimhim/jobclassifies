		...
		<div class="col-md-8 col-md-offset-2">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<div class="col-md-12">
						<form class="form-horizontal" role="form" method="POST" action="{{ route('job.save') }}">
							{{csrf_field()}}
								...
								<div class="col-md-9">
									...
									<select name="job_categories" class="form-control" id="job_categories">
										...
										@foreach ($array_job_categories as $key=>$category)
											<option value="{{$key}}" <?php echo ($key==old('job_categories'))?'selected':'';?>>{{$category}}</option>
										@endforeach
									</select>
							...
							<div class="form-group{{ $errors->has('job_priority') ? ' has-error' : '' }}">
								...
								<div class="col-md-9">
									<?php 
										$job_priority = array(1=>'Urgent',0=>'Normal');
									?>
									<select name="job_priority" class="form-control" id="job_categories">
										<option value="">-Select Priority-</option>
										@foreach ($job_priority as $key=>$priority)
										<option value="{{$key}}" <?php echo ($key==old('job_priority'))?'selected':'';?>>{{$priority}}</option>
										@endforeach
									</select>
